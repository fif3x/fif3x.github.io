...
Free and Open Source Software(FOSS) written mainly by @fif3x
...
int main() {
	...
	helpers h; // instances
	_STARTUP_ su; // instances

	if (prefs.debugMode) {
		std::cout << "Starting up..." << std::endl;
	}
	su._initTerminal_(); // initialize terminal
	if (prefs.debugMode) {
	...
	while (true) {
		...
		if (cmd == "exit" || cmd == "Exit") {
			...
			std::cout << "18. pr - Use previous command" << std::endl;
			std::cout << "19. resetPreferences - Reset all preferences" << std::endl;
			std::cout << "20. ucf - Use UCF (Ultimate Calculator by Fifex)" << std::endl;
			std::cout << "-------------------------------------------------------------------------" << std::endl;
			...
			IP = arg1;
			std::string placeholder = arg2;
			winCmd = (cmd + ' ' + IP + placeholder).c_str();
			linCmd = (cmd + ' ' + IP + placeholder).c_str();
			macCmd = (cmd + ' ' + IP + placeholder).c_str();
